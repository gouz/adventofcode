...
const racers = (await loadInputFile("2015/14", "input"))
	.split("\n")
	.map(
		(line) =>
			line
...
console.log(Math.max(...racers.map((racer) => run(racer, 2503).distance)));
...
console.log(points);
console.log(Math.max(...Object.values(points)) - 1);
