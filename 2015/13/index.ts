...
const input = await loadInputFile("2015/13", "input");
...
input
	...
	.forEach(([_, giver, gainOrLose, amount, receiver], _o) => {
		...
		names[giver][receiver] = Number(amount) * (gainOrLose === "lose" ? -1 : 1);
	});
...
console.log(`Part 1: ${Math.max(...[...new Set(sums(names))])}`);
...
console.log(`Part 2: ${Math.max(...[...new Set(sums(names))])}`);
